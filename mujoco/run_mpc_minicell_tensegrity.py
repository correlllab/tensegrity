#!/usr/bin/env python3
"""MPC controller for shoulder+elbow mini-cell tensegrity arm."""
    ...
    if args.viewer:
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "Try: ../.venv/bin/mjpython run_mpc_minicell_tensegrity.py --viewer"
            ) from exc
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="MPC controller for mini-cell tensegrity arm")
    p.add_argument("--model", default="arm_minicell_tensegrity.xml", help="Path to MuJoCo XML model")
    ...
    p.add_argument("--target-x", type=float, default=0.14, help="Target center x position")
    p.add_argument("--target-z", type=float, default=0.74, help="Target center z position")
    p.add_argument("--target-radius", type=float, default=0.03, help="Moving target radius")
